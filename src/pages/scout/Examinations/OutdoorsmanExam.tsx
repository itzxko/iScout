...
const OutdoorsmanExam = () => {
  const { outdoorsmanQuiz, getOutdoorsmanQuiz } = useQuiz();
  const { user } = useAuth();
  ...
  const getResults = async () => {
    console.log("pwede");
    try {
      let url = `http://localhost:8080/api/quiz-attempts?userId=${user?._id}&rank=outdoorsman`;
  ...
  const submitAnswers = async () => {
    if (user) {
      if (outdoorsmanQuiz.length === Object.keys(answers).length) {
        try {
    ...
    <>
      ...
      <div className="w-full flex flex-col items-center justify-center py-12 px-4 font-host-grotesk">
        <div className="w-full lg:w-3/6 flex flex-col items-center justify-center space-y-8">
          ...
          {outdoorsmanQuiz.map((question: any, index: number) => (
            <div
...
export default OutdoorsmanExam;
